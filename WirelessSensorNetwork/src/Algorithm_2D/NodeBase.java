...
public class NodeBase {
    ...
    /**
     * Simulates node localization in 2D space and returns average error of n simulations
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        double radioRange=35;            //input your desired radio range in % of the grid size
        ...
        int percentAnchors=40;           //input your desired number of anchors in % of all nodes
...
//          System.out.println("\n"+i+"\n");
            NodeBase nb= new NodeBase(maxX, maxY, radioRange, nNodes, percentAnchors, degHu);
            nb.init(isIter);
...
//          System.out.println(nb.determineClcFraction());
    ...
    public NodeBase(int maxX, int maxY, double rr, int nNodes, int percentAnchors, boolean degHu){
...
//        System.out.println("\n\nNUMBER OF UNKNOWNS: "+ unknowns.size()+"\n");
    ...
    public void fillAnchors(int numAnchors){
        IntStream.range(0, numAnchors).forEach(i-> knowns.add(new Anchor(maxX, maxY)));
//        System.out.println("fill Anchors done");

    }

    public void fillUnknowns(int numUnknowns){
        IntStream.range(0, numUnknowns).forEach(i-> unknowns.add(new UnknownNode(maxX, maxY, RADIORANGE)));
